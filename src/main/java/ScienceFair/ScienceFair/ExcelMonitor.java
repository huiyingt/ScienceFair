...
public class ExcelMonitor {
	...
	public static void main(String[] args) {
		...
		File file = new File("C:\\Users\\aidan\\OneDrive\\Documents\\Full Stack Development\\VLC-SCFSF18-DevOps\\27Oct2025\\ScienceFair\\src\\main\\java\\SchoolScienceFair.xlsx");
		...
		try (FileInputStream fis = new FileInputStream(file);
				...
				for (int i = 1; i <= sheet.getLastRowNum(); i++) {
					...
					if (!isCellEmpty(avgCell) && judge1 >= 0 && judge2 >= 0) {
						double correctAvg = Math.round(((judge1 + judge2) / 2.0) * 10.0) / 10.0;
	...
	private static double getCellNumericValue(Cell cell) {
		if (cell == null || cell.getCellType() != CellType.NUMERIC) {
			return -1; // Return -1 to indicate invalid or missing numeric value
		}
